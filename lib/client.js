...
Client.prototype.moneyIn = function (_opts) {
  ...
  if (_opts.cardNumber) {
    var cardType = creditCardType(_opts.cardNumber);
    if (cardType.type && cardType.type[0] === 'visa') {
      _opts.cardType = constants.CARD_TYPE.VISA;
    } else if (cardType.type && cardType.type[0] === 'master-card') {
      _opts.cardType = constants.CARD_TYPE.MASTERCARD;
    } else {
      _opts.cardType = constants.CARD_TYPE.CB;
    }
  }
