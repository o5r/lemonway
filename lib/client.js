...
Client.prototype.mapError = function(code, message) {
  ...
  return new errors.LemonwayError(code, message);
};
...
Client.prototype.getWalletDetailsBatch = function (ip, _opts) {
  var opts = _.merge({
    version: '2.0',
  }, {
    wallets: _opts.wallets.map((wallet) => ({
      EmailExtId: {
  ...
  return this._request('GetWalletDetailsBatch', ip, opts).then(results => {
    if (_.get(results, 'wallets.GetWalletDetailsResult')) {
      return _.get(results, 'wallets.GetWalletDetailsResult');
    }
...
Client.prototype.createVirtualIBAN = function(ip, _opts) {
  const opts = {
    ..._opts,
    version: '1.0',
  };
