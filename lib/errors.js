...
var errorClasses = { LemonwayError: LemonwayError };

errorClasses = _.reduce(errors.concat(duplicatedErrors), function (reducer, error) {
  var _e = LemonwayErrorInherit(error.className, error.codes, error.message, error.description);

  return _(reducer).set(error.className, _e)
}, errorClasses);
