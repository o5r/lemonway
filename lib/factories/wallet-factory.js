...
const VirtualIBAN = require('../models/VirtualIBAN');
...
function WalletFactory (lemonway) {
  ...
  this.getBatch =
  this.getWalletDetailsBatch = function (ip, wallets, _opts) {
    const opts = _opts || {};

    opts.wallets = wallets;

    return lemonway._client.getWalletDetailsBatch(ip, opts)
      .bind(this)
      .then(function (data) {
        return data.reduce((result, walletOrError) => {
          const wallet = _.get(walletOrError, 'WALLET');
          if (wallet) {
            result.wallets.push(new this.Wallet(wallet))
          } else {
            const error = _.get(walletOrError, 'E');

            result.errors.push(lemonway._client.mapError(error.Code, error.Msg));
          }
  ...
  this.createVirtualIBAN = async function (ip, wallet, _opts) {
    const opts = _.assign({
      wallet: _.get(wallet, 'id', wallet)
    }, _opts);

    const data = await lemonway._client.createVirtualIBAN(ip, opts);

    return new VirtualIBAN(data);
  };
  ...
  var instanceMethods = {
    ...
    'createVirtualIBAN': ['createVirtualIBAN'],
    'moneyOut': ['moneyOut'],
